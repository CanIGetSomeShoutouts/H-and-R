...
function _create(req, res) {
    ...
    usersService.create(req.model)
        .then(data => {
            if (req.model.userType === 'Therapist') { 
                userId = data
                const fullName = `${req.model.firstName} ${req.model.lastName}`
                emailsService.sendAlertEmailToAdmin(fullName, userId, process.env.ADMIN_ADDRESS)
            }
            if (req.model.userType === 'Supporter') 
            {
                res.sendStatus(200)
                return 
            }

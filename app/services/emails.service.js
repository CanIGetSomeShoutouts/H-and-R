...
module.exports = {
  ...
  sendAlertEmailToAdmin: _sendAlertEmailToAdmin,
...
function _sendAlertEmailToAdmin(fullName, userId, adminEmail) {
  let subject = "A therapist has just signed up!"
  let body = `<p> ${fullName} has become a therapist!</p>
  <p>Click the therapist name below to go to their page.</p>
  <a href="${process.env.ORIGIN}/admin/users/detail/${userId}" style="background-color:#0000ff;border-radius:3px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:16px;line-height:44px;text-align:center;text-decoration:none;width:150px;">Therapist</a>`

  return _send(adminEmail, userId, subject, body)
}
...
function _sendRegistrationConfirmation(toAddress, userId, toName) {
  ...
  <p>Thank you for registering to Heal & Recover. In order to complete your registration, please click the confirmation
      ...
      <a href="${process.env.ORIGIN}/users/${userId}" style="background-color:#0000ff;border-radius:3px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:16px;line-height:44px;text-align:center;text-decoration:none;width:150px;">Confirm Email</a>`
...
function _sendSupportInvitation(fromName, toName, toAddress, inviteId) {
  ...
  let body = `<p>Hello,</p>
    ...
    <a href="${process.env.ORIGIN}/anon/confirm-support/${inviteId}" style="background-color:#0000ff;border-radius:3px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:16px;line-height:44px;text-align:center;text-decoration:none;width:150px;">Accept Invitation</a>`
  return _send(toAddress, toName, subject, body)
}